<template>
  <div class="container collect mt-8 mt-sm-10">
    ...
    <template v-if="collectData.length > 0">
      <div
        class="row row-cols-1 row-cols-md-2 row-cols-lg-3 row-cols-xl-4 mb-5"
      >
        <button
          type="button"
          @click.prevent="goToOneProduct(`${product[0].id}`, $event)"
          class="card text-primary bg-dark col mb-md-4 d-block"
          v-for="(product, index) in collectData"
          :key="product.id"
          @mouseover="hightLight(`${index}`, 'open')"
          @mouseleave="hightLight(`${index}`, 'close')"
        >
          <router-link
            :to="`/one_product/${product[0].id}`"
            class="
              product_img
              card-img-top
              animation_hover
          ...
          >
            ...
            />
            <span class="badge bg-danger p-1" v-if="product[0].popular > 2"
              >熱門商品</span
            >
            <button
              type="button"
              class="d-block animation_hover collect_btn"
              :class="`collect_btn${index}`"
              title="點擊移除收藏"
              @click.prevent="toggleCollect(product[0].id, index)"
              data-clickType="btn"
            >
              <i
                class="bi bi-heart-fill fs-5"
                v-if="collectIdData.includes(product[0].id)"
                data-clickType="btn"
              ></i>
              <i
                class="bi bi-heartbreak-fill fs-5 text-primary"
                data-clickType="btn"
                v-if="!collectIdData.includes(product[0].id)"
              ></i>
          ...
          <div class="card-body w-100">
            ...
            <div class="d-flex justify-content-between align-items-end mb-3">
              <del style="opacity: 0.8"> {{ product[0].origin_price }} 元</del>
              <strong
                >特價<span class="text-danger text-end fs-4">
...
export default {
  ...
  methods: {
    ...
    getColletProduct () {
      this.collectIdData =
        JSON.parse(localStorage.getItem('collectIdData')) || []
      this.collectIdData.forEach((item) =>
        this.collectData.push(
          this.products.filter((product) => product.id === item)
        )
      )
    ...
    toggleCollect (id, index) {
      ...
      if (collectIndex === -1) {
        ...
        localStorage.setItem(
          'collectIdData',
          JSON.stringify(this.collectIdData)
        )
